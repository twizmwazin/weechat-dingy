...
fn main() {
    ...
    let tcp = TcpStream::connect(&server_addr).and_then(move |stream| {
        println!("Ah");
        ...
        let task = tx.clone().send(Box::new(init_command)).and_then(|tx| {
        ...
        let task2 = 
        rx.fold(sink, move |sink, command: Box<Command+Send>| {
